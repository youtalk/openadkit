...
**The image manifest `aib/x5h-rootfs.aib.yml` is the source of truth for every file on the board.** Its `add_files` mapping installs the unit files, the environment file, the DDS config, the scenario, the launch stub and the relay nodes, all flattened into one directory beside the units, plus the board smoke `scripts/x5h-stack-smoke.sh` and its metric helper `scripts/x5h-stop-metrics.awk` into `/usr/sbin`. Hand-installed board copies go stale the moment the repository changes: after any edit, re-copy the file to the board and `systemctl daemon-reload` — or rebuild the image, which is what the mapping is for.

The smoke and its awk go to `/usr/sbin`, beside `rpmsg-eth-ifup.sh`, and NOT to `/usr/local/sbin`: `/usr/local` is one of the two paths `aib` rejects outright (see the note above `add_files:` in the manifest), because this rootfs ships it as a symlink into a `/var` that is empty at image-build time. They must land in the same directory as each other, because `drive` reads the awk as `$(dirname "$0")/x5h-stop-metrics.awk`. `/usr` is read-only at runtime, so iterating on either file between image builds means copying the **pair** somewhere writable (`/var/tmp`) and pointing `X5H_SMOKE` at that copy — an explicit override, which is what keeps a stale hand-copy from quietly grading a board that a fresh image would grade differently.
