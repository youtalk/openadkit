...
- `mrm_no_stop_metrics` — the MRM chain completed but the stop distance could not be computed: the `kinematic_state` capture, the staged `x5h-stop-metrics.awk`, or a stamped fault record to anchor t=0 is missing. There is no fallback anchor — the availability capture's first `autonomous: false` record predates engagement and is never substituted for the fault stamp. Check `drive-probe/` for `kinematic_state.txt` and confirm `x5h-stop-metrics.awk` sits next to the smoke script.
