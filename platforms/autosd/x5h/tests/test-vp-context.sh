...
mkdir -p "$tmp/ov/include/onnxruntime"; : > "$tmp/ov/include/onnxruntime/onnxruntime_cxx_api.h"
...
[ -L "$tmp/ctx/ort/lib/libonnxruntime.so" ] && [ -L "$tmp/ctx/ort/lib/libonnxruntime.so.1" ] || fail symlinks
[ -f "$tmp/ctx/ort/include/onnxruntime/onnxruntime_cxx_api.h" ] || fail headers_copied
echo "TEST_PASS $name"
